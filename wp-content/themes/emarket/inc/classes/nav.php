...
if( !class_exists("Emarket_PBR_bootstrap_navwalker") ){
class Emarket_PBR_bootstrap_navwalker extends Walker_Nav_Menu {
   ...
   /**
    ...
    public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
        ...
        if( isset($item->text_customize) && !empty($item->text_customize) ){
            switch ( $item->text_customize ) {
                case 'text_new':
                    $text_customize  = esc_html__('New', 'emarket');
                    break;

                case 'text_hot':
                    $text_customize  = esc_html__('Hot', 'emarket');
                    break;

                case 'text_featured':
                    $text_customize  = esc_html__('Featured', 'emarket');
                    break;
    ...
    public function genMegaMenuByConfig( $item, $depth ){
        ...
        global $emarket_megamenu; 

        $post = get_post( $item->megamenu_profile ); $emarket_megamenu = 'true';
        $content = do_shortcode( $post->post_content ); 
        $emarket_megamenu  = null;
