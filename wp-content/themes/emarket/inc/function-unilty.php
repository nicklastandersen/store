...
if( !function_exists("emarket_fnc_wpml_language_buttons") ){
   function emarket_fnc_wpml_language_buttons(){
  ...
  */
function emarket_fnc_get_header_layouts(){
    $path = get_template_directory().'/header-*.php';
    $files = glob( $path  );
    $headers = array( '' => esc_html__('Default', 'emarket') );
    if( count($files)>0 ){
        foreach ($files as $key => $file) {
            $header = str_replace( "header-", '', str_replace( '.php', '', basename($file) ) );
            $headers[$header] = esc_html__( 'Header', 'emarket' ) . ' ' .str_replace( '-',' ', ucfirst( $header ) );
        }
    }
  ...
  */
function emarket_fnc_get_footer_profiles(){
    
    $footers_type = get_posts( array('posts_per_page' => -1, 'post_type' => 'footer') );
    $footers = array(  '' => esc_html__('None', 'emarket') );
 ...
 */
function emarket_fnc_get_menugroups(){
 ...
 *
 */
function emarket_fnc_cst_skins(){
 ...
 * 
 */
function emarket_fnc_render_post_content( $footer ){

  global $emarket_wpopconfig; 

	$post = get_post( $footer );
  $emarket_wpopconfig['type'] = 'footer';
	if($post){
    	echo do_shortcode( $post->post_content ); 
  }
  
  $emarket_wpopconfig['type'] = '';
 ...
 */
if(!function_exists('emarket_fnc_makeid')){
    function emarket_fnc_makeid($length = 5){
...
if(!function_exists('emarket_fnc_excerpt')){
    //Custom Excerpt Function
    function emarket_fnc_excerpt($limit,$afterlimit='[...]') {
...
function emarket_fnc_get_widget_block_styles(){
   return array(  'default' , 'primary', 'danger' , 'success', 'warning', 'coffe', 'bluesky' );
}


if(!function_exists('emarket_fnc_related_post')){
    function emarket_fnc_related_post( $relate_count = 4, $posttype = 'post', $taxonomy = 'category' ){
