...
?>
	<div class="widget widget-products widget-product-tabs woocommerce <?php echo ($el_class!='')?' '.esc_attr( $el_class ):''; ?>">
			...
			<?php foreach ($list_query as $key => $li) { ?>
				<div class="tab-pane<?php echo ($__count==0)?' active':''; ?>" id="<?php echo esc_attr($key).'-'.$_id; ?>">
					<div class="grid-wrapper">
							<?php
								$loop = emarket_fnc_woocommerce_query($key,$number);
								if( $number == -1 )
