...
<p class="entry-link">
    <a  href="<?php echo esc_url( $post->link ) ?>" class="vc_read_more btn btn-default"
        title="<?php echo esc_attr( sprintf( esc_html__( 'Permalink to %s', "emarket" ), $post->title_attribute ) ); ?>"<?php echo esc_html( $this->link_target ); ?>><?php esc_html_e( 'Read more', "emarket" ) ?></a>
</p>
<?php endif; ?>